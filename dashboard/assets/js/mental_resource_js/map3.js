...
var promises = [];
    ...
    Promise.all(promises).then(function (values) {  //use Promise.all to load map and data
     ...
     var data = new Map(data1.slice(1).map(([Site_Count, state, county]) => [state + county, +Site_Count]))
      ...
      radius = d3.scaleSqrt([0, d3.quantile([...data.values()].sort(d3.ascending), 0.985)], [0, 7]);
      pop = Object.assign(new Map(pop.map((d) => [d.id, +d.population])));
      pop.title = "Population (million)";
...
// Add legend items
var legendItemWidth = 45;  // Width of the rectangle
...
legend.selectAll(".legend-text")
    ...
    .attr("y", legendItemHeight + 10) // Position below the line and rectangle
    .text(d => formatNumber(d.text))
    .attr("fill", "black")
    .attr("font-size", 12)
...
legend.append("text")
    ...
    .attr("y", -10) // Adjust the y-position to place it above the legend items
    .text(pop.title)
    .attr("fill", "black") // Optional: set the text color
    .style("font-size", "14px") // Optional: set the font size
...
///// county ledgend/////////
radius1 = d3.scaleSqrt([0, d3.quantile([...data.values()].sort(d3.ascending), 0.985)], [0, 8]);
var radius_legend1 = svg.selectAll('radius-legend1')
        ...
        .attr('class', 'radius_legend1')
            ...
            .text("Mental Health Resource Distribution & Population Density by States");
  ...
  .attr("class", "state-group");
      stateGroups.each(function(stateData) {
        ...
        group.append("path")
          ...
          .attr("stroke", "white") // Initial state border color
          .attr("stroke-width", 1)
          .append("title")
      .text(d => `${d.properties.name}, ${format(pop.get(d.id))}`);
          //.on("click", clicked);
...
function updateMap(selectedState_map) {
    ...
    var title = "Mental Health Resource Distribution & Population Density by County";
    mapTitle.text(title)
