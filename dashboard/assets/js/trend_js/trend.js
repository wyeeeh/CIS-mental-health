...
// tips
  ...
  const show_line = (d) => {
    let line = ChartArea.selectAll(".myline")
      .data([1])
      .join("line")
      .attr("class", "myline");
    line
  ...
  const hide_line = () => {
    ChartArea.selectAll(".myline").attr("y2", 0);
  };
  // 比例尺x y  和颜色
...
.style('font-family', 'Inter');
  ...
  // 画两条线标志疫情开始和结束
  let dates = [new Date("2020-1-30"), new Date("2023-5-2")];
  dates.forEach((d, i) => {
...
function add_filter({ div, optionsList, changeEvent, top, right, label }) {
  ...
  $("select").multipleSelect({
    ...
    // 设置默认选中项为 "anxiety"
    $('select').multipleSelect('setSelects', ['anxiety']);
    select_value = ['anxiety']; // 更新select_value数组
...
function drawBarChart(data, selectedKeywords, divId, timeRanges,color) {
  ...
  // 标题数组
  const titles = ["Post COVID-19", "During COVID-19", "After COVID-19"];
...
async function init() {
  ...
  function type_selected(seleted_items) {
    ...
    drawBarChart(filter_data, seleted_items, "barChartDiv", [
      [d3.timeFormat("%Y-%m-%d")(from_date), '2020/1/30'],
      ['2020/1/31', '2023/5/23'],
      ['2023/5/24', d3.timeFormat("%Y-%m-%d")(to_date)]
    ],color);
  }
  ...
  function resetCharts() {
    ...
    let initialFilter = ["anxiety"]; // 初始筛选关键词，根据您的应用进行调整
    ...
    drawBarChart(line_data, initialFilter, "barChartDiv", [
      [initialFromDate, '2020/1/30'],
      ['2020/1/31', '2023/5/23'],
      ['2023/5/24', initialToDate]
    ], color);
  }
